function students(data) {

    let obj = {};

    for (let line of data) {
        if (line.includes(': ')) {
            let [course, capacity] = line.split(': ');
            capacity = Number(capacity);
            if (!obj[course]) {
                obj[course] = { capacity, students: [] };
            } else {
                obj[course].capacity += capacity;
            };
        } else if (line.includes(' email ')) {
            let [userName, credits, email, courseJoined] = line
                .replace('[', '*')
                .replace('] with email ', '*')
                .replace(' joins ', '*')
                .split('*')
                
            if (obj.hasOwnProperty(courseJoined)) {
                if (obj[courseJoined].capacity > 0) {
                    obj[courseJoined].students.push({ userName, credits, email, courseName: courseJoined });
                    obj[courseJoined].capacity -= 1;
                };
            };
        };
    };
    let capacitySort = Object.entries(obj).sort((a,b) => Object.keys(b[1].students).length - 
    Object.keys(a[1].students).length);

    for(let el of capacitySort) {
        console.log(`${el[0]}: ${el[1].capacity} places left`);
        el[1].students.sort((a,b) => b.credits - a.credits).forEach(el => {
            console.log(`--- ${el.credits}: ${el.userName}, ${el.email}`);
        });
    };
};
students(['JavaBasics: 2', 'user1[25] with email user1@user.com joins C#Basics', 'C#Advanced: 3', 'JSCore: 4', 'user2[30] with email user2@user.com joins C#Basics', 'user13[50] with email user13@user.com joins JSCore', 'user1[25] with email user1@user.com joins JSCore', 'user8[18] with email user8@user.com joins C#Advanced', 'user6[85] with email user6@user.com joins JSCore', 'JSCore: 2', 'user11[3] with email user11@user.com joins JavaBasics', 'user45[105] with email user45@user.com joins JSCore', 'user007[20] with email user007@user.com joins JSCore', 'user700[29] with email user700@user.com joins JSCore', 'user900[88] with email user900@user.com joins JSCore'])